...
def snap_to_nearest_node(latlon: Tuple[float, float]) -> Dict[str, Any]:
    ...
    node_y = float(G_orig.nodes[node]["y"])
    node_x = float(G_orig.nodes[node]["x"])
    
    dist = haversine(lat, lon, node_y, node_x)
    
    if dist > 500: # 500 meters max snap distance
        return {"error": "Too far from road network (limit 500m). Please click closer to a road."}
